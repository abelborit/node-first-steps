...
const { getUserByIdCallback } = require("./js-bases/03-callbacks");

const idCallback = 2;
getUserByIdCallback(idCallback, function (error, response) {
  if (error) {
...
const { getUserByIdArrow } = require("./js-bases/04-arrow");

const idArrow = 2;
getUserByIdArrow(idArrow, function (error, response) {
  if (error) {
    throw new Error(error);
  }
...
const { buildMakePerson } = require("./js-bases/05-factory");
...
const getPokemonById = require("./js-bases/06-promises");
